...
Proyectil::Proyectil(int x, int y, int direccion)
    : x(x), y(y), direccion(direccion), direccionY(0), velocidad(1), activo(true)
{
...
// Proyectil general
Proyectil::Proyectil(int x, int y, int dx, int dy, const std::string& spr)
    : x(x), y(y), direccion(dx), direccionY(dy), velocidad(1), activo(true), sprite(spr) {}
...
//getters
int  Proyectil::getX() const { 
    return x; 
}
int  Proyectil::getY() const { 
    return y; 
}
...
std::string Proyectil::getSprite() const { 
    return sprite; 
}